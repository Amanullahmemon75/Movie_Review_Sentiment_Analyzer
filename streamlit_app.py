...
# Ensure the correct paths to your models
...
def load_models():
    try:
        if not os.path.exists(vectorizer_path):
            raise FileNotFoundError(f"Vectorizer model file not found at: {vectorizer_path}")
        if not os.path.exists(svc_model_path):
            raise FileNotFoundError(f"SVC model file not found at: {svc_model_path}")
        
        vectorizer = joblib.load(vectorizer_path)
        svc_model = joblib.load(svc_model_path)
        st.success("Models loaded successfully!")
        return vectorizer, svc_model
...
# Load the models
vectorizer, svc_model = load_models()

if vectorizer and svc_model:
